...
class BBProject:
    ...
    @staticmethod
    def __get_inherit_package_names(client: BBClient, inherit_class_name: str) -> List[str]:
        ...
        for package in recipe_info:
            ...
            ret.append(client.data_store_connector_cmd(datastore_index, DataStoreFunctions.GET_VAR, _FrequentlyUsedVarNames.PACKAGE_NAME))
        return ret
...
class BBPackage:
    ...
    def from_name(client: BBClient, name: str) -> "BBPackage":
        ...
        instance.description: str = instance.get_var(_FrequentlyUsedVarNames.DESCRIPTION)
        instance.license: str = instance.get_var(_FrequentlyUsedVarNames.LICENSE)
        instance.source_uris: List[str] = instance.get_var(_FrequentlyUsedVarNames.SRC_URI).split()
        instance.package_epoch: str = instance.get_var(_FrequentlyUsedVarNames.PACKAGE_EPOCH)
        instance.package_version: str = instance.get_var(_FrequentlyUsedVarNames.PACKAGE_VERSION)
        instance.package_revision: str = instance.get_var(_FrequentlyUsedVarNames.PACKAGE_REVISION)
        instance.package_depends: List[str] = instance.get_var(_FrequentlyUsedVarNames.DEPENDS).split()
        instance.package_runtime_depends: List[str] = instance.get_var(_FrequentlyUsedVarNames.RDEPENDS).split()
        instance.install_files: List[str] = instance.get_var(_FrequentlyUsedVarNames.FILES).split()

        various_files: List[str] = instance.get_var(_FrequentlyUsedVarNames.BBINCLUDED).split()
        instance.conf_files: List[str] = [file for file in various_files if file.endswith(_FileParams.CONF_SUFFIX) or "/conf/" in file]
        instance.bbclass_files: List[str] = [file for file in various_files if file.endswith(_FileParams.BBCLASS_SUFFIX)]
        instance.recipe_files: List[str] = [file for file in various_files if not file.endswith((_FileParams.CONF_SUFFIX, _FileParams.BBCLASS_SUFFIX)) and not "/conf/" in file]

        instance.all_variable_names: List[str] = list(instance.client.data_store_connector_cmd(instance.__datastore_index, DataStoreFunctions.KEYS))
        instance.tasks: List[str] = [var for var in instance.all_variable_names if instance.get_var_flag(var, _FrequentlyUsedVarNames.FLAG_TASK)]
        
        depends_graph, runtime_depends_graph, task_depends_graph = BBPackage.__generate_depends_graph(instance.package_name, instance.client)
    ...
    def get_var_flag(self: "BBPackage", var_name: str, flag: str) -> Any:
        ...
        """
        return self.client.data_store_connector_cmd(self.__datastore_index, DataStoreFunctions.GET_VAR_FLAG, var_name, flag)
    ...
    def __get_datastore_index(client: BBClient, name: str) -> Any:
        ...
        global_overrides: str = client.data_store_connector_cmd(global_datastore_index, DataStoreFunctions.GET_VAR, _FrequentlyUsedVarNames.OVERRIDES)

        local_datastore_index: int = client.data_store_connector_cmd(global_datastore_index, DataStoreFunctions.CREATE_COPY).dsindex
        client.data_store_connector_cmd(global_datastore_index, DataStoreFunctions.SET_VAR, _FrequentlyUsedVarNames.OVERRIDES, global_overrides + ":" + name) # TODO: if dunfell or older, use "_" instead of ":"
        return local_datastore_index

    @staticmethod
    def __generate_depends_graph(package_name: str, client: BBClient) -> Tuple[networkx.DiGraph, networkx.DiGraph, networkx.DiGraph]:
        ...
        def monitor(bbclient_:BBClient, event: DepTreeGeneratedEvent):
            ...
            depends_tree = event.depgraph[_FrequentlyUsedVarNames.FLAG_DEPENDS]                # This is corresponding to DEPENDS 
            runtime_depends_tree = event.depgraph[_FrequentlyUsedVarNames.FLAG_RDEPENDS_PN]    # This is maybe corresponding to RDEPENDS, but I'm not sure.
            task_depends_tree = event.depgraph[_FrequentlyUsedVarNames.FLAG_TASK_DEPENDS]          # This is task depends.
        ...
        BBPackage.__create_node(runtime_depends_tree_root, runtime_depends_tree, package_name, [])
        task_depends_tree_root: networkx.DiGraph = networkx.DiGraph()
        BBPackage.__create_node(task_depends_tree_root, task_depends_tree, package_name + ".do_build", [])
